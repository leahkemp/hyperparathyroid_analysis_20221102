rule pbrun_vqsr_indel:
    ...
    shell:
        "/opt/parabricks/3.7/parabricks/pbrun vqsr --in-vcf {input.vcf} --out-vcf {output.vcf} --out-recal {output.recal} --out-tranches {output.tranches} {params.indeltranche} {params.resources} {params.other} &> {log}"